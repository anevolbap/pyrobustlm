...
cdef inline double _chi_sum(
    ...
    if family == FAM_BISQUARE:
        ...
        # top-level entry point before we drop the GIL. Hoist per-case
        # constants out of the inner loop and precompute 1/s.
        j = <int>(tuning[0]) - 1
        if j < 0:
            j = 0
        elif j > 5:
...
cdef inline double _ggw_chi_sum(
    ...
    """Sum of GGW chi(r[i]/s) over i, with all per-case constants
...
cdef inline void _wgt_zinv(
    ...
    if family == FAM_BISQUARE:
        ...
        # the analytical wgt = exp(-((|x|-c)^b)/(2a)). The only b values
        # in robustbase's default table are 1.0 (cases 1..3) and 1.5
        # (cases 4..6). Hardcoding those two branches saves a libm pow()
        # call per element (~2x on the hot inner loop).
        j = <int>(tuning[0])
        if j < 1:
            j = 1
        elif j > 6:
        ...
        if b_t == 1.0:
            ...
            # User-supplied b; fall back to libm pow().
            for i in range(n):
                xi = r[i] / s
                ax = xi if xi >= 0 else -xi
                if ax <= r_t:
